...
### 有効化
**Origin Trial** 状態（Chrome 148〜150）。Origin Trial に登録済みであればフラグ不要。
未登録環境では Chrome フラグ `chrome://flags/#canvas-draw-element` で有効化できる。

Origin Trial トークンは `<meta>` タグで埋め込む:
...
### Canvas サイズ・Renderer 設定

Three.js は生の canvas を直接触らず、renderer を通してサイズとピクセル密度を指定する。
canvas は renderer が生成し、`document.body.appendChild( renderer.domElement )` で追加する。
